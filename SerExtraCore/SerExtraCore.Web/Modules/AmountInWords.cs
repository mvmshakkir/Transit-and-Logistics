...
namespace SerExtraCore.Web.Modules
{
    ...
    {
        public  string GetAmount(decimal amountfrom)
        {
            //double amount = 0;
            //double.TryParse(amountfrom.ToString(), out amount);
            string isNegative = "";

            string number = Convert.ToDouble(amountfrom).ToString("N3");
            if (amountfrom < 0)
            {
                isNegative = "Minus ";
